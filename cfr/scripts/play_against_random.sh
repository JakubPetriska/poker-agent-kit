...
# Agent trained using CFR is pitted against required number of random agents (given by game's player count).
#
...
if [ "$#" != 1 ]; then
  echo 'Usage: ./play_against_random.sh {game_file_path}'
  exit 1
fi
...
do
  PLAYERS="${PLAYERS} Random_${i} "${WORKSPACE_DIR}/scripts/agents/agent_random.${GAME_NAME}.sh""
done
